...
export function StandingOfferFiltersBar({ categories }: { categories: { name: string; count: number }[] }) {
    ...
    <div className="panel p-4 space-y-4">
      ...
      >
        <input
          ...
          placeholder="Search title, commodity, supplier…"
      ...
      <div className="flex flex-wrap items-center gap-2 text-xs">
        <button
          ...
          className={`chip cursor-pointer ${activeOnly ? "chip-green" : ""}`}
        >
          Active only
        </button>
      </div>
